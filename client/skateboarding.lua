...
AddEventHandler('Skateboard:Clear', function()
	Skateboard.Clear()
end)

AddEventHandler('Skateboard:Start', function()
	Skateboard.Clear()
    Skateboard.Start()
end)
...
Skateboard.Start = function()
	...
	while DoesEntityExist(Skateboard.Entity) and DoesEntityExist(Skateboard.Driver) do
		...
		if distanceCheck <= Config.SkateLoseConnectionDistance then
...
Skateboard.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < Config.SkateLoseConnectionDistance then
		local overSpeed = (GetEntitySpeed(Skateboard.Entity)*3.6) > Config.SkateMaxSpeedKmh
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if not IsEntityInAir(Skateboard.Entity) then
				...
				boost = (Config.SkateMaxJumpHeight * duration / 250.0)
				if boost > Config.SkatemaxJumpHeight then boost = Config.SkatemaxJumpHeight end
