...
RegisterNetEvent('rollerskating:client:putOnBlades', function()
	...
	Wait(3000)
	if getSex() then
		SetPedComponentVariation(PlayerPedId(),6,RollerConfig.RollerSlotHomme,RollerConfig.RollerColorHomme,2)
	else
		SetPedComponentVariation(PlayerPedId(),6,RollerConfig.RollerSlotFemme,RollerConfig.RollerColorFemme,2)
	end
...
RegisterCommand("rollerbladestop", function()
	actif = false
	Wait(1500)
...
function RCCar.Start()
	...
	while DoesEntityExist(RCCar.Entity) and DoesEntityExist(RCCar.Driver) do
		...
		if distanceCheck <= RollerConfig.LoseConnectionDistance then
...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < RollerConfig.LoseConnectionDistance then
		local overSpeed = (GetEntitySpeed(RCCar.Entity)*3.6) > RollerConfig.MaxSpeedKmh
		local overSprint = (GetEntitySpeed(RCCar.Entity)*3.6) > RollerConfig.MaxSpeedSprintKmh
		...
		-- Input Control longboard
			...
			if IsControlPressed(0, 22) and Attached then
				...
				if not IsEntityInAir(RCCar.Entity) then
					...
					boost = RollerConfig.maxJumpHeigh * duration / 250.0
					if boost > RollerConfig.maxJumpHeigh then boost = RollerConfig.maxJumpHeigh end
					if(Attached) then
